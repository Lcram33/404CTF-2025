...
# It contains a key discovery that will be useful to solve the challenge.
...
class Saturn:
    ...
    def Encrypt(self, x):
        ...
        for r in range(self.N):
            ...
            # print("AddKey", state.hex(' ').upper())
            state = self.SubBytes(state)
            # print("SubBytes", state.hex(' ').upper())
            state = self.Permute(state)
            # print("Permute", state.hex(' ').upper())
            # print()
        return state
...
print("Chiffrement selon la position des blocs :")
print(f"{block.hex(' ').upper()} => {enc.hex(' ').upper()} | Compare : {(bytes([x ^ y for x, y in zip(enc, base_enc)])).hex(' ').upper()}")
...
block_str = changing_byte + 28 * '0' + changing_byte
block = bytes.fromhex(block_str)
enc = S.Encrypt(block)
print(f"{block.hex(' ').upper()} => {enc.hex(' ').upper()} | Compare : {(bytes([x ^ y for x, y in zip(enc, base_enc)])).hex(' ').upper()}")

# Well, looks like we can map all the plaintext bytes to the ciphertext bytes ! (a "p-box")